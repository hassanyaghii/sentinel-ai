        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium uppercase tracking-tighter flex items-center gap-1">
              <Database className="w-3 h-3" /> config_snapshots
            </p>
          </div>
        </div>
        {selected && (
        ...
        <div className="w-80 flex flex-col space-y-4 shrink-0">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
            ...
            <form onSubmit={handleRunExtraction} className="space-y-2">
              ...
              <button 
                ...
                className="w-full py-2.5 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl flex items-center justify-center space-x-2 shadow-lg hover:bg-blue-700 active:scale-95 disabled:bg-slate-200 transition-all"
              >
                {isExtracting ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Download className="w-3.5 h-3.5" />}
                <span>{isExtracting ? 'Extracting...' : 'Fetch Config'}</span>
              </button>
            </form>
          </div>
          ...
          <div className="flex-1 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col p-3">
            <div className="flex items-center justify-between mb-3 px-1">
              ...
              <button onClick={fetchSnapshots} className="p-1 hover:bg-slate-50 rounded transition-colors"><RefreshCw className={`w-3 h-3 ${isLoading ? 'animate-spin' : ''}`} /></button>
            </div>
            <div className="flex-1 overflow-auto space-y-2 custom-scrollbar pr-1">
              {configs.map(c => (
                <div 
                  ...
                  className={`p-3 rounded-xl border transition-all cursor-pointer relative group ${selectedId === c.id ? 'bg-slate-900 text-white border-slate-800 shadow-md' : 'bg-slate-50 border-slate-100 hover:border-slate-300'}`}
                >
                  <div className="flex items-center justify-between mb-1">
                    <p className="font-bold text-[11px] truncate">{c.hostname || 'Unknown Host'}</p>
                    <div className={`w-1.5 h-1.5 rounded-full ${selectedId === c.id ? 'bg-blue-400' : 'bg-slate-300'}`}></div>
                  </div>
                  <div className="flex justify-between items-center">
                    <p className={`text-[9px] font-mono ${selectedId === c.id ? 'text-slate-400' : 'text-slate-400'}`}>{c.ip_address}</p>
                    <p className={`text-[8px] font-black uppercase ${selectedId === c.id ? 'text-blue-300' : 'text-slate-300'}`}>{new Date(c.created_at).toLocaleDateString()}</p>
                  </div>
                </div>
              ))}
            </div>
